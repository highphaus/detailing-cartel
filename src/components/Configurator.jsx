...
export default function Configurator({ selectedCoating, setSelectedCoating, coatingLayers, setCoatingLayers, specs }) {
  return (
    <section id="configurator" className="max-w-7xl mx-auto px-6 py-24 border-t border-white/5 font-sans">
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-12 items-center">
        <div className="lg:col-span-7 bg-[#0C0C0E] border border-white/5 rounded-3xl p-8 sm:p-10 space-y-8 shadow-sm reveal-left">
          <div className="space-y-2">
            <span className="text-[#E9001A] text-[9px] font-bold uppercase tracking-widest block">LAB WORKBENCH</span>
            <h3 className="text-2xl sm:text-3xl font-black uppercase tracking-tight text-white">SIMULATE COATING MATRIX</h3>
          </div>

          <div className="space-y-3">
            <label className="block text-[9px] uppercase tracking-widest font-bold text-[#9E9EA7]">Select Crystalline Formula</label>
            <div className="grid grid-cols-3 gap-3">
              {['Ceramic', 'Borophene', 'Nano Diamond'].map((tech) => (
                <button 
                  key={tech} onClick={() => setSelectedCoating(tech)}
                  className={`py-3 rounded-xl border text-[10px] font-bold uppercase tracking-wider transition-all duration-300 ${
                    selectedCoating === tech 
                      ? 'bg-[#E9001A] text-white border-[#E9001A] shadow-[0_4px_12px_rgba(233,0,26,0.2)]' 
                      : 'bg-white/2 border-white/10 text-white hover:border-[#E9001A]/40'
                  }`}
                >
                  {tech}
                </button>
          ...
          <div className="space-y-4">
            <div className="flex justify-between text-xs font-bold uppercase tracking-wider text-[#9E9EA7]">
              ...
              <span className="text-[#E9001A]">{coatingLayers} {coatingLayers === 1 ? 'Layer' : 'Layers'} Matrix</span>
            </div>
            <div className="relative flex items-center">
              <input 
                type="range" min="1" max="5" value={coatingLayers} 
                onChange={(e) => setCoatingLayers(Number(e.target.value))}
                className="custom-range-input w-full accent-[#E9001A] bg-white/10 h-2.5 rounded-lg appearance-none cursor-pointer" 
              />
            </div>
          </div>
          ...
          <div className="grid grid-cols-3 gap-4 pt-6 border-t border-white/5">
            <div>
              ...
              <span className="text-sm sm:text-base font-bold text-white mt-1 block">{specs.hardness}</span>
            </div>
            <div>
              <span className="text-[9px] text-[#9E9EA7] uppercase tracking-widest font-bold block">Curing Protocol</span>
              <span className="text-sm sm:text-base font-bold text-white mt-1 block">{specs.curingTime} Hours</span>
            </div>
            <div>
              <span className="text-[9px] text-[#9E9EA7] uppercase tracking-widest font-bold block">Beading Angle</span>
              <span className="text-sm sm:text-base font-bold text-[#E9001A] mt-1 block">{specs.hydrophobicAngle}° Struct</span>
            </div>
          </div>
        </div>

        <div className="lg:col-span-5 space-y-6 reveal-right">
          <span className="text-[#E9001A] font-black uppercase tracking-[0.4em] text-xs">SPECIFICATION RESULT</span>
          <h2 className="text-3xl sm:text-5xl font-black uppercase tracking-tight text-white leading-none">
            ESTIMATED <br /> FORMULA <br /> SPECS
          </h2>
          ...
          <div className="bg-[#0C0C0E] border border-[#E9001A]/15 rounded-3xl p-6 relative overflow-hidden">
